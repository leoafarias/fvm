...
| Migration coverage | `scripts/manual-migration-test.sh` exercises an isolated FVM 4.0.1 cache upgrade, while `docs/pages/documentation/guides/manual-smoke-test.md` covers current config, cache, links, and command flows. | Extend these patterns only if the final FVM-4-to-FVM-5 installation cutover needs stateful coverage without touching a developer's real home or cache. |
