...
''');
    ...
    test('should find melos.yaml in parent directory but not update', () async {
      ...
      createProjectConfig(ProjectConfig(flutter: '3.10.0'), subDir);
...
''');
    ...
    test(
      'should not modify melos.yaml with existing non-FVM sdkPath',
      () async {
        final testDir = tempDirs.create();
        // Create test project
        createPubspecYaml(testDir);
...
''');
    ...
    test('should calculate correct relative path for nested melos', () async {
      ...
      createProjectConfig(ProjectConfig(flutter: '3.10.0'), nestedDir);
...
''');
    ...
    test('should not update melos settings when config disables it', () async {
      ...
      createPubspecYaml(testDir);
      createProjectConfig(ProjectConfig(updateMelosSettings: false), testDir);
...
''');
    ...
    test(
      'should detect existing FVM path but not update without confirmation',
      () async {
        final testDir = tempDirs.create();
        // Create test project
        createPubspecYaml(testDir);
...
''');
    ...
    group('with simulated user input', () {
      test(
        'should update melos.yaml when user confirms (simulated Yes)',
        () async {
          final testDir = tempDirs.create();
          createPubspecYaml(testDir);
          createProjectConfig(ProjectConfig(flutter: '3.10.0'), testDir);
...
''');
          ...
          expect(
            logger.outputs.any(
              (msg) => msg.contains('Added FVM Flutter SDK path to melos.yaml'),
            ),
            isTrue,
          );
        },
...
''');
        ...
        expect(
          logger.outputs.any(
            (msg) => msg.contains('Updated FVM Flutter SDK path in melos.yaml'),
          ),
          isTrue,
        );
      });
