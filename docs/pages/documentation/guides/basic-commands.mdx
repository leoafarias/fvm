...
Removes all installed Flutter SDK versions from FVM's versions cache. FVM
configuration and the Git cache are preserved.
...
```bash
# Remove all cached Flutter SDK versions with a confirmation prompt
fvm destroy

# Remove all cached Flutter SDK versions without confirmation (dangerous)
fvm destroy --force
```

### Warning

This action is irreversible and removes all cached Flutter SDKs. It does not
remove FVM configuration or the Git cache.
