...
run. They are never applied automatically. A cached patch that is the
recommended upgrade target is kept even if `fvm list` labels it `Unused`.
Because FVM only knows projects it has previously recorded, unused SDK
