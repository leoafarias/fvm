...
`fvm list` uses it to mark cached SDKs that no known project pins and that are not selected globally as `Unused`. Because only projects you have run FVM in are recorded, that label is a hint, not proof an SDK is safe to delete.
