...
`fvm list` uses it to mark cached SDKs that no known project pins and that are not selected globally as `Unused`. `fvm cleanup` uses the same list for unused-SDK removal and cached patch upgrades. Because only projects you have run FVM in are recorded, those results are a hint, not proof an SDK is safe to delete.
