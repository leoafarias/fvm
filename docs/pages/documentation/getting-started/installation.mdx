...
<Callout type="info">
  If the installer can't update a shell config—because the file is missing, unreadable, or read-only—you'll see warnings such as `Skipping ~/.bashrc; file not found.` or `Skipping ~/.bash_profile; failed to write changes (file may be read-only).` Copy the PATH export the script prints and paste it into any writable rc file for your shell.
</Callout>
