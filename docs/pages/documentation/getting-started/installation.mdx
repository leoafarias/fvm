...
<Callout type="info">
  On read-only setups (Nix/Home Manager), the installer may warn `Skipping ~/.bash_profile; could not update file (likely read-only). Add PATH manually.` Just paste the shown PATH export into a writable rc file yourself.
</Callout>
