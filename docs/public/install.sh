...
if [[ "$OS" == "macos" ]] && sysctl -n sysctl.proc_translated 2>/dev/null | grep -q '^1$'; then
  # Prefer the native arm64 binary even if the current shell runs under Rosetta (x86_64)
...
if [[ -z "$FVM_VERSION" ]]; then
  ...
  if ! LATEST_URL=$(curl -fsSL -o /dev/null -w '%{url_effective}' \
    https://github.com/leoafarias/fvm/releases/latest 2>/dev/null); then
    error "Failed to detect latest FVM version from GitHub releases.

Check your network connection or specify a version explicitly:
  ./install.sh 3.2.1
"
  fi
  ...
  if [[ -z "$FVM_VERSION" ]]; then
    error "Latest release tag from GitHub was empty.
...
ensure_safe_install_path() {
  local target="${1%/}"
  case "$target" in
    ""|"/"|"/bin"|"/sbin"|"/usr"|"/usr/bin"|"/usr/sbin"|"/usr/local"|"/usr/local/bin")
      error "Refusing to use unsafe installation path: $target"
      ;;
  esac
}

ensure_safe_install_path "$FVM_DIR"
ensure_safe_install_path "$FVM_DIR_BIN"

# Clean existing installation
if [[ -d "$FVM_DIR_BIN" ]]; then
  info "Removing existing FVM installation..."
  if [[ -f "$FVM_DIR_BIN/fvm" ]]; then
    rm -f "$FVM_DIR_BIN/fvm" || error "Failed to remove existing FVM binary."
  fi
  if [[ -d "$FVM_DIR_BIN/src" ]]; then
    rm -rf "$FVM_DIR_BIN/src" || error "Failed to remove existing FVM source directory."
  fi
fi
...
if [[ ! -x "$FVM_DIR_BIN/fvm" ]]; then
  if ! chmod +x "$FVM_DIR_BIN/fvm"; then
    error "Failed to make fvm executable: $FVM_DIR_BIN/fvm
...
Check mount options:
  mount | grep \$(df \"$FVM_DIR_BIN\" | tail -1 | awk '{print \$1}')

Try a different installation directory:
  FVM_DIR=/opt/fvm ./install.sh"
  fi
fi
...
update_shell_config() {
  ...
  if [[ -w "$config_file" ]]; then
    if ! grep -Fxq "$export_command" "$config_file"; then
      {
        printf '\n# FVM\n'
        printf '%s\n' "$export_command"
      } >> "$config_file"
